...
class Interceptor{
	...
	void InstallInterceptor()
	{
		...
		m_OriginalFunctionByteCodePrefix
			= (BYTE*) VirtualAlloc(NULL,m_countBytes+JMP_SIZE,MEM_COMMIT,PAGE_EXECUTE_READWRITE);

		DWORD dwOriginalProtection;
		VirtualProtect(m_OriginalFunctionAddr, m_countBytes, PAGE_EXECUTE_READWRITE,
			&dwOriginalProtection); // unlock
		...
		VirtualProtect(m_OriginalFunctionAddr, m_countBytes, dwOriginalProtection,
			&dwOriginalProtection);	// restore lock
